...
|意味・用途|HTMLtag|解説|
|-|-|-|
|見出し|h1～6|見出しに使用すること。<br><br>dt要素との使い分けに迷うことがある場合<br>以下の基準で判断すること。<br><ul><li>そのTextがPageの目次として適しているか</li><li>そのTextに対するcontentがあるか</li></ul>HTML5の"section tag"で文書の階層構造を表している場合、<br>見出しlevelの数字は1～6のどれを使っても仕様上問題はないが、<br>codeを読んだ際に階層levelがわかりにくいため、<br>"section tag"を使っていたとしても階層levelに合わせて数字を増やすこと。|
...
|list|ul > li|順序に意味がないlist（箇条書き）に使用すること。<br><br>文章を確認し、箇条書きといえる内容（端的で長文を含まない）かどうか、<br>順序に意味があるかどうかを判断してMarkupすること。|
|順序list|ol > li|順序に意味があるlist（箇条書き）に使用すること。<br><br>"ul tag"によるMarkupの考え方に加えて順序に意味があるかどうかで使い分ける。<br>番号が付いている＝"ol tag"ではないため、<br>まずは箇条書きとして適切かどうかを判断した後に順序に意味があるかどうかを判断し使うこと。|
|定義list|dl > dt + dd|用語とその説明から構成される文章に使用すること。<br><br>用語以外にも、新着情報の年月日（dt tag）と内容（dd tag）や対話型の文章の中で話者（dt tag）とその内容（dd tag）にも使用できる。<br>汎用性が高く、用途も広いため便利なHTMLtagなため、<br>他に適切なHTMLtagがないかを考えてから使うこと。|
|注釈・細目|small|免責・警告・法的規制・著作権・license要件などを注釈・細目に使用すること。<br>一般的なWeb siteでよくあるものとしては、Copywritが挙げられる。|
|記事|article|このHTMLtagでMarkupされた内容が独立したcontentとして成り立つものに使用すること。<br><br>Blog記事・News記事・新着情報などが対象になります。 独立しているかどうか判断に困ることがありますので、以下を判断基準とすること。<br><ul><li>RSS feedで配信するcontentか</li><li>そのcontentだけを抜き取ってWordなどに貼り付けたとき、<br>何のEvent・Objectについて言及したcontentなのか理解できるか</li></ul>それでも困った場合には使用しないこと。|
|余談・補足情報|aside|文章の本筋ではないが、<br>余談・補足程度で軽く触れている文章や関連する広告・商品に使用すること。<br><br>文章の意図や書き手の考えを汲み取らなければ正しく使用できないため、使用が難しいHTMLtagの一つ。<br>以下の基準で判断し、それでも使用に迷った場合は使わないこと。<ul><li>このHTMLtagでMarkupされた文章が削除された場合にcontentが破綻しないか。</li></ul>|
|Formの項目名|label|"input tag"や"select tag"など入力要素に対する項目名に使用すること。<br><br>このHTMLtagでMarkupされた内容をClickしたときに、<br>Focusが対象の要素に移るようにfor属性を指定すること。|
...
　※"p tag"や"h" levelのHTMLtagが入ってくる為。
