...
class HUV_sns_settings {
	 ...
	 * 使用するシェアボタンの表示設定
	 */
	public function sns_settings( $slug ) {
		// 値を取得
		...
		foreach ($snsarr as $key => $value) {
			...
			// if ( get_option( 'huv_admin_settings' )[$value] == "" ) {
			// 	echo '<input type="checkbox" id="sns_check_'.$value.'" name="huv_admin_settings['.$value.']" value="'.$value.'">';
			// } else {
			// 	echo '<input type="checkbox" id="sns_check_'.$value.'" name="huv_admin_settings['.$value.']" value="'.$value.'" checked>';
			// }
			if ( get_option( $slug )[$value] == "" ) {
				echo '<input type="checkbox" id="sns_check_'.$value.'" name="'.$slug.'['.$value.']" value="'.$value.'">';
			} else {
				echo '<input type="checkbox" id="sns_check_'.$value.'" name="'.$slug.'['.$value.']" value="'.$value.'" checked>';
			}
	...
	public function setting_views( $slug ) {
		// 入力項目のセクションを追加します。
		...
		// add_settings_section( 'huv_admin_settings_section_id', '', '', 'huv_admin_settings' );
		add_settings_section( 'huv_admin_settings_section_id', '', '', $slug );
		...
		// add_settings_field( 'sns', '使用するシェアボタン', array( $this, 'sns_settings' ), 'huv_admin_settings', 'huv_admin_settings_section_id' );
		add_settings_field( 'sns', '使用するシェアボタン', array( $this, 'sns_settings' ), $slug, 'huv_admin_settings_section_id' );
	}

	/**
	 * 設定ページのHTMLを出力する処理
	 */
	public function create_admin_page( $slug ) {
		// データベーステーブル"options"から名前を指定してオプションの値を取得する
		// $this->options = get_option( 'huv_admin_settings' );
		...
		echo "<div class=\"wrap\">\n";
			...
			echo "<form method=\"post\" action=\"options.php\">\n";
				...
				// settings_fields( 'huv_admin_settings' );
				settings_fields( $slug );
				// 入力項目を出力します(設定ページのslugを指定)。
				// do_settings_sections( 'huv_admin_settings' );
	...
	public function setting_save( $slug ) {
		...
		// register_setting( 'huv_admin_settings', 'huv_admin_settings', array( $this, 'save_valid' ) );
		register_setting( $slug, $slug, array( $this, 'save_valid' ) );
	}
